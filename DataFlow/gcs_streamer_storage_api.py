...
class WriteToGcs(beam.DoFn):
    def __init__(self, output_path):
        ...
        self.blob_path='/'.join(self.output_path.split('/')[3:])
    def process(self, element):
        import json
        import uuid
        ...
        try:
            ...
            for item in batch_data:
                multiline_string += f'{json.dumps(item)}\n'
            
            
            # logging.info(f'multiline_string: {multiline_string}')
            file_name=f'{self.blob_path}_{str(uuid.uuid4())}_{current_time}_{shard_id}.jsonl'
            bucket = storage_client.bucket(self.bucket_name)
            blob = bucket.blob(file_name)
            blob.upload_from_string(multiline_string.encode('utf-8'))
...
def run():
    ...
    google_cloud_options.job_name = 'pubsub-to-gcs'
    pipeline_options.view_as(beam.options.pipeline_options.WorkerOptions).num_workers = 8  # Replace 5 with your desired number
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        output_file ='gs://dflow-test-b1/output2/part'
